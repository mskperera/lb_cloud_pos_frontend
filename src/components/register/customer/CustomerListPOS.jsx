...
export default function CustomerSelectionModal({ 
  ...
  }, [showCustomerList, currentPage, rowsPerPage, selectedFilterBy.value, searchValue.value]);
  ...
  }, [showCustomerList, currentPage, rowsPerPage, loadCustomers]);
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === "ArrowDown") {
        ...
        e.preventDefault();
        setSelectedRowIndex(prev => (prev > 0 ? prev - 1 : -1));
  ...
  }, [showCustomerList, customers, selectedRowIndex, isLoading, onCustomerSelectHandler]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-1 xs:p-2 sm:p-4" 
      ...
      <div className="relative w-full h-full lg:h-auto lg:max-w-6xl lg:max-h-[92vh] bg-white rounded-none lg:rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col">
        ...
        {/* Filters */}
        <div className="px-3 xs:px-4 sm:px-6 lg:px-8 py-4 xs:py-5 bg-gray-50 border-b flex-shrink-0">
          ...
          <div className="lg:hidden space-y-3">
            <select
              ...
              className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-sky-500 text-sm"
            >
            ...
            <div className="flex justify-between items-center text-sm text-gray-600">
              <span>{totalRecords} customers found</span>
              <DaisyUIPaginator
            ...
            <div className="grid grid-cols-12 gap-6">
              <div className="col-span-3">
                ...
                <select
                  value={selectedFilterBy.value}
                  onChange={(e) => setSelectedFilterBy({ value: +e.target.value })}
                  className="w-full px-5 py-3.5 rounded-2xl border border-gray-300 focus:ring-2 focus:ring-sky-500 text-sm"
                >
                  {filterByOptions.map(o => (
                    <option key={o.id} value={o.id}>{o.displayName}</option>
                  ))}
              ...
              <div className="col-span-7">
                ...
                <input
                  ...
                  onChange={(e) => setSearchValue({ value: e.target.value })}
                  onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
                  placeholder="Enter customer name, code, phone, email..."
                  className="w-full px-5 py-3.5 rounded-2xl border border-gray-300 focus:ring-2 focus:ring-sky-500 text-sm"
                />
              </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 xs:px-4 lg:px-6 py-4 bg-white min-h-[320px]">
          <div className="hidden lg:block bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {isLoading ? (
                  <tr><td colSpan={6} className="text-center py-16"><div className="inline-block animate-spin w-9 h-9 border-4 border-sky-500 border-t-transparent rounded-full" /></td></tr>
                ) : customers.length === 0 ? (
                  <tr><td colSpan={6} className="text-center py-16 text-gray-500">No customers found</td></tr>
                ) : (
                  customers.map((c, i) => (
                    <tr
                      key={c.contactId}
                      tabIndex={0}
                      className={`hover:bg-sky-50 cursor-pointer transition-all outline-none ${selectedRowIndex === i ? 'bg-sky-100 ring-2 ring-sky-500 ring-inset' : ''}`}
                      onClick={() => setSelectedRowIndex(i)}
                      onDoubleClick={() => { onCustomerSelectHandler(c); setShowCustomerList(false); }}
                    >
          ...
          <div className="lg:hidden space-y-3">
            {isLoading ? (
              <div className="text-center py-12"><div className="inline-block animate-spin w-8 h-8 border-4 border-sky-500 border-t-transparent rounded-full" /></div>
            ) : customers.length === 0 ? (
              ...
              customers.map((c, i) => (
                <div
                  key={c.contactId}
                  className={`bg-white rounded-2xl border border-gray-200 p-4 cursor-pointer transition-all ${selectedRowIndex === i ? 'ring-2 ring-sky-500 bg-sky-50' : ''}`}
                  onClick={() => setSelectedRowIndex(i)}
                >
                  <div className="flex justify-between">
                    <div>
                      <div className="font-semibold">{c.contactName}</div>
                      <div className="text-sm text-gray-500">{c.contactCode}</div>
                    </div>
                    <div className="text-right">
                      <div className="font-medium text-sky-600">{c.mobile}</div>
                    </div>
                  </div>
                  <button
                    onClick={(e) => { e.stopPropagation(); onCustomerSelectHandler(c); setShowCustomerList(false); }}
