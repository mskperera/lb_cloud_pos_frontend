...
const StockAdjustmentList = ({ inventoryId }) => {
  ...
  const [stockPriceChanges, setPriceChanges] = useState([]);
  const [newUnitPrice, setNewUnitPrice] = useState("");
  const [newUnitCost, setNewUnitCost] = useState("");
  const [changeReason, setChangeReason] = useState("");
  const [showPriceChangePanel, setShowPriceChangePanel] = useState(false);
  const [showZeroStockQtyData, setShowZeroStockQtyData] = useState(false);
  const [product, setProduct] = useState(null);
  const [isSelectBatchManually, setIsSelectBatchManually] = useState(false);
  ...
  const loadDetails = async () => {
    ...
    try {
      ...
      setStockInfo(stockInfoRes.data.results[0]);
      setIsSelectBatchManually(stockInfoRes.data.outputValues.isSelectBatchManually);
    } catch (error) {
  ...
  const handleSaveAdjustment = async () => {
    if (adjustmentQty === 0 || !adjustmentReason) {
      showToast("warning", "Exception", "Please provide a valid quantity and reason.");
      return;
    }
    setLoading(true);
    const adustmentTypeId = adjustmentType === "add" ? 1 : 2;
  ...
  const loadStockAdjustments = async (stockBatchId) => {
    try {
      ...
      setStockAdjustments(res.data.results[0]);
    } catch (err) {
  ...
  const loadPriceChange = async (stockBatchId) => {
    try {
      ...
      setPriceChanges(res.data.results[0]);
    } catch (err) {
      console.error("Error fetching stock adjustments:", err);
    }
  };
  ...
  const handleSavePriceChange = async () => {
    if (!newUnitPrice || !newUnitCost) {
      showToast("warning", "Exception", "Please provide valid price and cost values.");
      return;
    }
    ...
    try {
      setLoading(true);
      const res = await updatePrice(priceChangeData);
      if (res.data.error) {
  ...
  const renderStockInfo = () => {
    if (loading) {
      return <div className="text-center text-gray-600">Loading stock batch details...</div>;
    }
    ...
    return (
      <div className="p-4">
        <div className="flex items-center mb-4">
          ...
          />
          <label htmlFor="showZeroStockQtyData" className="ml-2 text-sm text-gray-700">
            Show Zero Quantity Stocks
          </label>
        </div>
        <div className="bg-gray-50 rounded-lg p-4 overflow-x-auto">
          <h3 className="font-semibold text-lg text-gray-800 mb-4">Stock Batch Details</h3>
          <table className="w-full border-collapse text-sm">
            <thead className="bg-gray-50 sticky top-0 z-10 border-b border-gray-300">
              <tr className="text-left text-gray-700 font-medium">
                ...
                <th className="px-4 py-3 w-32">Batch Queue No</th>
                <th className="px-4 py-3 w-36">Expiration Date</th>
                <th className="px-4 py-3 w-36">Production Date</th>
                <th className="px-4 py-3 w-28 text-right">Unit Price</th>
                <th className="px-4 py-3 w-28 text-right">Unit Cost</th>
                <th className="px-4 py-3 w-36">Created</th>
                <th className="px-4 py-3 w-80">Actions</th>
              </tr>
            </thead>
            <tbody>
              {stockInfo
                ...
                .map((item, index) => {
                  let isStockOutOrder;
                  if (isSelectBatchManually)
                    isStockOutOrder = !!item.isBatchReleased === true && item.qty > 0;
                  else
                    isStockOutOrder = !!item.currentBatchFlag;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">{item.batchQueueNumber}</td>
                      <td className="px-4 py-3">
                        {item.expDate ? formatUtcToLocal(item.expDate, true) : "-"}
                      </td>
                      <td className="px-4 py-3">
                        {item.prodDate ? formatUtcToLocal(item.prodDate, true) : "-"}
                      </td>
                      <td className="px-4 py-3 text-right">{formatCurrency(item.unitPrice)}</td>
                      <td className="px-4 py-3 text-right">{formatCurrency(item.unitCost)}</td>
                      <td className="px-4 py-3">{formatUtcToLocal(item.createdDate_utc)}</td>
                      <td className="px-4 py-3">
                        <div className="flex space-x-2">
                          {isSelectBatchManually ? (
                            <>
                              {item.isBatchReleased ? (
                                <button
                                  onClick={() => handleReleasePrompt(item, true)}
                                  className={`inline-flex items-center px-4 py-2 rounded-md text-sm font-medium text-white bg-red-500 hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 ${
                                    parseInt(item.qty) === 0 ? "opacity-50 cursor-not-allowed" : ""
                                  }`}
                                ...
                                <button
                                  onClick={() => handleReleasePrompt(item, false)}
                                  className="inline-flex items-center px-4 py-2 rounded-md text-sm font-medium text-white bg-sky-500 hover:bg-sky-600 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-opacity-50"
                                >
                                  Release Batch
                                </button>
                              )}
                          ...
                          <button
                            ...
                            className="inline-flex items-center px-4 py-2 rounded-md text-sm font-medium text-white bg-green-500 hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50"
                          >
                            Change Price
                          </button>
                          <button
                            onClick={() => handleAdjustStock(item, index)}
                            className="inline-flex items-center px-4 py-2 rounded-md text-sm font-medium text-white bg-yellow-500 hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-opacity-50"
                          >
                            Adjust Stock
                          </button>
                        </div>
    ...
    return (
      <div className="p-4 bg-gray-50 rounded-lg mt-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          <div className="flex flex-col">
            <span className="font-semibold text-gray-700 mb-2">Current Quantity</span>
            <span className="text-gray-800">{selectedBatch.qty}</span>
          </div>
          <div className="flex flex-col">
            <span className="font-semibold text-gray-700 mb-2">Adjustment Type</span>
            <div className="flex gap-4 items-center">
              ...
              <label className="flex items-center">
                <input
                  ...
                  className="h-4 w-4 text-sky-500 focus:ring-sky-500 border-gray-300"
                />
                <span className="ml-2 text-sm text-gray-700">Deduct</span>
              </label>
            </div>
          </div>
          <InputField
          ...
          <div className="flex justify-end gap-2 col-span-1 sm:col-span-2 lg:col-span-4 mt-4">
            <button
            ...
            <button
              ...
              className="inline-flex items-center px-4 py-2 rounded-md text-sm font-medium text-gray-700 bg-gray-200 hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-opacity-50"
            >
              Cancel
            </button>
          </div>
          ...
          <table className="w-full border-collapse text-sm">
            <thead className="bg-gray-50 sticky top-0 z-10 border-b border-gray-300">
              <tr className="text-left text-gray-700 font-medium">
                ...
                <th className="px-4 py-3 w-36">Created</th>
                <th className="px-4 py-3 w-32 text-right">Current Stock Qty</th>
                <th className="px-4 py-3 w-32 text-right">Adjustment Amount</th>
                <th className="px-4 py-3 w-32 text-right">Final Stock Qty</th>
                <th className="px-4 py-3 w-32">Adjustment Type</th>
                <th className="px-4 py-3 w-40">Reason Name</th>
                <th className="px-4 py-3 w-48">Other Remarks</th>
              </tr>
            </thead>
            <tbody>
              {stockAdjustments.map((item, index) => {
                const rowColor = item.adjustmentTypeName === "Deduction" ? "bg-red-100" : "bg-green-100";
                const finalStockQty =
                  item.adjustmentTypeId === 1
                    ? item.existingStockQty + item.adjustedQty
                    : item.existingStockQty - item.adjustedQty;
                return (
                  <tr key={index} className={`${rowColor} hover:bg-opacity-80 transition-colors`}>
                    <td className="px-4 py-3">{item.stockAdjustmentId}</td>
                    <td className="px-4 py-3">{formatUtcToLocal(item.createdDate_UTC)}</td>
                    <td className="px-4 py-3 text-right">{item.existingStockQty}</td>
                    <td className="px-4 py-3 text-right">
                      {item.adjustmentTypeId === 1 ? item.adjustedQty : -item.adjustedQty}
                    </td>
                    <td className="px-4 py-3 text-right">{finalStockQty}</td>
                    <td className="px-4 py-3">{item.adjustmentTypeName}</td>
                    <td className="px-4 py-3">{item.adjustmentReasonName}</td>
                    <td className="px-4 py-3">{item.adjustmentReasonOtherRemark}</td>
                  </tr>
                );
              })}
            </tbody>
  ...
  const renderPriceChangePanel = () => {
    ...
    return (
      <div className="p-4 bg-gray-50 rounded-lg mt-4">
        <h3 className="font-semibold text-lg text-gray-800 mb-4">
          Price Change for Batch: {selectedBatch.batchNo}
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          <InputField
            id="newUnitPrice"
          ...
          <table className="w-full border-collapse text-sm">
            <thead className="bg-gray-50 sticky top-0 z-10 border-b border-gray-300">
              <tr className="text-left text-gray-700 font-medium">
                <th className="px-4 py-3 w-24">ID</th>
                <th className="px-4 py-3 w-36">Created</th>
                <th className="px-4 py-3 w-32 text-right">Old Unit Price</th>
                <th className="px-4 py-3 w-32 text-right">New Unit Price</th>
                <th className="px-4 py-3 w-32 text-right">Old Unit Cost</th>
                <th className="px-4 py-3 w-32 text-right">New Unit Cost</th>
                <th className="px-4 py-3 w-48">Change Reason</th>
              </tr>
            </thead>
            <tbody>
              {stockPriceChanges.map((item, index) => {
                ...
                return (
                  <tr key={index} className="hover:bg-gray-100 transition-colors">
                    <td className="px-4 py-3">{item.priceChangeLogId}</td>
                    <td className="px-4 py-3">{formatUtcToLocal(item.CreatedDate_UTC)}</td>
                    <td className={`px-4 py-3 text-right ${priceChanged ? "bg-red-100" : ""}`}>
                      {item.oldUnitPrice}
                    </td>
                    <td className={`px-4 py-3 text-right ${priceChanged ? "bg-red-100" : ""}`}>
                      {item.newUnitPrice}
                    </td>
                    <td className={`px-4 py-3 text-right ${costChanged ? "bg-red-100" : ""}`}>
                      {item.oldUnitCost}
                    </td>
                    <td className={`px-4 py-3 text-right ${costChanged ? "bg-red-100" : ""}`}>
                      {item.newUnitCost}
    ...
    <div className="p-6">
      {showDialogReleaseBatch && (
        <ConfirmDialog
          isVisible={true}
          message={
            stockBatchInfoToRelaseConfirm.isStopRelease
              ? "Are you sure you want to stop this batch?"
              : "Are you sure you want to release this batch?"
          }
